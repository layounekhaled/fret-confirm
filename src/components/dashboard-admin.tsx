...
const statusColors: Record<string, string> = {
  ...
  en_preparation: '#14b8a6', emballée: '#10b981', prete_expedition: '#84cc16',
...
const statusLabels: Record<string, string> = {
  ...
  envoyee_ecotrack: 'Envoyée', erreur_ecotrack: 'Erreur Eco.', rupture_stock: 'Rupture',
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          title="CA confirmé"
          value={`${data.caConfirmee.toLocaleString('fr-FR')} DA`}
          icon={DollarSign}
          color="emerald"
        />
        <StatCard
          title="Erreurs Ecotrack"
          value={data.ecotrackErrors.toString()}
          icon={AlertTriangle}
          color="red"
        />
      </div>
            ...
            <div className="space-y-3">
              ...
              {data.topShops.map((shop, i) => (
                <div key={shop.shopId} className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{shop.count} commandes</Badge>
                </div>
              ))}
            </div>
          </CardContent>
            ...
            <div className="space-y-3">
              ...
              {data.topAgents.map((agent, i) => (
                <div key={agent.userId} className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{agent.count} confirmations</Badge>
                </div>
              ))}
            </div>
          </CardContent>
...
function StatCard({ title, value, icon: Icon, color }: { title: string; value: string; icon: React.ElementType; color: string }) {
  const bgColor = color === 'red' ? 'bg-red-50' : 'bg-emerald-50'
  const iconColor = color === 'red' ? 'text-red-600' : 'text-emerald-600'
  return (
    <Card>
